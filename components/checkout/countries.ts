...
/**
 * The list of countries offered in the checkout form. `value` is what gets
 * submitted to the license server, `label` is what the user sees.
 */
...
const ALIASES: Record<string, string> = {
  ...
  "south korea": "Korea Sout",
  "korea, republic of": "Korea Sout",
  "north korea": "Korea North",
  "czechia": "Czech Republic",
  "netherlands": "Netherlands",
  "the netherlands": "Netherlands",
  "holland": "Netherlands",
  "united arab emirates": "United Arab Erimates",
  "uruguay": "Uraguay",
...
const BY_KEY: Map<string, string> = new Map();
for (const { value, label } of COUNTRIES) {
  BY_KEY.set(value.toLowerCase(), value);
  BY_KEY.set(label.toLowerCase(), value);
}

/**
 * Resolves a country name coming from an external source (e.g. a geo-IP
 * lookup) to one of the values accepted by the select. Returns `undefined`
 * when it cannot be matched, so callers never set a value the user cannot
 * see selected in the dropdown.
 */
export function resolveCountry(name: string | undefined | null): string | undefined {
  if (!name) return undefined;
  const key = name.trim().toLowerCase();
  return BY_KEY.get(key) ?? ALIASES[key];
}
