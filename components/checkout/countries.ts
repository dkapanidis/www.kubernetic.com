...
/**
 * Countries offered in the checkout form, as ISO 3166-1 alpha-2.
 *
 * `code` is what gets submitted to kubernetic-admin; `name` is only ever
 * displayed. Keeping the stored value a code (rather than a display name) means
 * nothing downstream has to match on spelling, and it is the format Stripe
 * expects for addresses and tax.
 */
...
/** EU member states, for the VAT rules in YourOrderSection. */
const EU = new Set([
...
export function isEuropeanCountry(code: string | undefined | null): boolean {
  return !!code && EU.has(code);
}
